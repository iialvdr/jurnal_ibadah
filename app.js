...
window.lastCity = "Menunggu GPS..."; // Variable baru untuk nama kota
...
window.openProfile = () => {
    ...
    // Tampilkan Nama Kota (Bukan Koordinat)
...
async function saveRecordToCloud(prayerId, status) {
    if (!currentUser) return;
    const dateKey = formatDateKey(currentDate);
    const userDocRef = doc(db, "users", currentUser.uid, "daily_records", dateKey);
    
    currentRecords[prayerId] = status;
    renderPrayers();
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            (pos) => {
                ...
                // Panggil Jadwal Sholat
                fetchJadwal(window.lastLat, window.lastLng);
                // Panggil Nama Kota (Reverse Geocoding)
...
// Fungsi Baru: Ubah Koordinat jadi Nama Kota
async function fetchCityName(lat, lng) {
    try {
        ...
        // Ambil data alamat (Prioritas: Kota -> Kabupaten -> Desa)
        const addr = data.address;
        const city = addr.city || addr.town || addr.village || addr.county || "Indonesia";
        
        window.lastCity = city; // Simpan ke global variable
...
window.togglePrayer = (id, locked) => {
    if (locked) return;
    const currentStatus = currentRecords[id] || false;
    saveRecordToCloud(id, !currentStatus);
};
...
function renderPrayers() {
    ...
    PRAYER_CONFIG.forEach((p, idx) => {
        ...
        if (p.type === 'wajib') { wajibTotal++; if (isDone) wajibDone++; }
        ...
        let wrapperClass = status.locked 
            ...
            : (isDone 
                ? 'cursor-pointer bg-white dark:bg-slate-800 border-emerald-500 shadow-md translate-x-1' 
        ...
        let iconClass = isDone ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400';
        let checkIcon = isDone 
            ? `<div class="bg-emerald-500 text-white rounded-lg p-1"><i data-lucide="check" class="w-4 h-4"></i></div>`
            : (status.locked ? `<i data-lucide="lock" class="w-4 h-4 text-slate-300"></i>` : `<div class="border-2 border-slate-200 dark:border-slate-600 rounded-lg w-6 h-6"></div>`);

        html += `
        <div onclick="togglePrayer('${p.id}', ${status.locked})" class="slide-up flex items-center justify-between p-4 rounded-2xl border transition-all duration-300 mb-3 ${wrapperClass}" style="animation-delay: ${idx * 50}ms">
            <div class="flex items-center gap-4">
                <div class="p-2 rounded-xl bg-slate-100 dark:bg-slate-700/50 ${iconClass}"><i data-lucide="${p.icon}" class="w-5 h-5"></i></div>
                <div>
                    <h3 class="font-bold text-base dark:text-slate-200 ${isDone ? 'text-emerald-700 line-through decoration-emerald-500/50' : ''}">${p.id}</h3>
    ...
    if (wajibTotal > 0 && wajibDone === wajibTotal && !checkTimeAvailability(prayerTimes.Subuh).locked) {
        html += `<div class="mt-6 p-4 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-2xl text-center shadow-lg animate-bounce-slow"><p class="font-bold">✨ Alhamdulillah Sempurna! ✨</p><p class="text-xs opacity-90">Pertahankan terus ya, Valdi!</p></div>`;
    }
