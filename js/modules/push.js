import { db, WEB_PUSH_VAPID_PUBLIC_KEY } from '../config.js';
import { doc, deleteDoc, setDoc } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js";
...
export async function syncPushSubscription(user, isEnabled) {
    ...
    try {
        ...
        if (!vapidKey) {
            console.warn('Web Push VAPID key belum diisi di js/config.js');
            return { supported: true, subscribed: false, reason: 'missing-vapid-key' };
        }
