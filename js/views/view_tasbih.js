...
<div id="tasbihView" class="hidden-force flex flex-col h-full absolute inset-0 z-50 transition-all duration-300 overflow-hidden">
    ...
    <div class="px-6 pb-2 pt-[calc(2rem+env(safe-area-inset-top))] flex justify-between items-center relative shrink-0 z-20">
    ...
    <div class="flex-1 flex flex-col items-center justify-center p-6 pb-20 space-y-6 relative">
        
        <div class="flex bg-white/40 dark:bg-slate-800/40 backdrop-blur-md rounded-2xl p-1.5 border border-white/40 dark:border-slate-700/30 shadow-sm z-10">
        ...
        <div id="dhikrDisplayArea" class="hidden text-center space-y-2 animate-[zoomIn_0.3s_ease-out] z-10 px-4">
            <h3 id="dhikrArabicDisplay" class="text-3xl font-bold font-serif text-slate-800 dark:text-white leading-loose"></h3>
            <p id="dhikrLatinDisplay" class="text-sm font-medium text-emerald-600 dark:text-emerald-400 italic"></p>
        </div>
        
        <button onclick="openDhikrMenu()" class="flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-100/50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 text-xs font-bold border border-emerald-200/50 hover:scale-105 transition z-10 backdrop-blur-sm">
            <i data-lucide="book-open" class="w-4 h-4"></i> Pilih Bacaan
        </button>

        <div onclick="countTasbih()" class="relative group cursor-pointer select-none tap-highlight-transparent z-0 mt-4">
    ...
    <div id="dhikrMenuModal" class="absolute inset-0 z-50 bg-black/60 backdrop-blur-sm hidden flex flex-col justify-end transition-opacity duration-300">
        <div class="bg-white dark:bg-slate-900 rounded-t-3xl p-6 h-[70%] flex flex-col animate-[slideUp_0.3s_ease-out]">
            <div class="flex justify-between items-center mb-6">
                <h3 class="font-bold text-lg text-slate-800 dark:text-white">Pilih Bacaan</h3>
                <button onclick="closeDhikrMenu()" class="p-2 bg-slate-100 dark:bg-slate-800 rounded-full text-slate-500"><i data-lucide="x" class="w-5 h-5"></i></button>
            </div>
            <div id="dhikrListContainer" class="flex-1 overflow-y-auto space-y-3 pb-10">
                </div>
        </div>
    </div>
</div>
